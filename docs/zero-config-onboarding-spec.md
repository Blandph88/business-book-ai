...
- **B3. Redaction = a BUYER TOGGLE + platform-executed redaction. NOT per-feature dev code** (unenforceable — the guarantee would only be as good as the laziest developer). Egress happens in the broker (`byok.ts`); BB is sealed and can't send or pick the tier. So the control + mechanism live in Freehold; the dev's only job is a *declaration*.
  - **B3-platform (Freehold broker) — the control + the mechanism:**
    ...
    - **Redaction runs in the broker** (tokenize before the `fetch`, detokenize the response locally): (a) **generic detection always** — emails / phones / URLs / cards (regex) + names (light on-device pass) — the enforceable floor even if the dev declared nothing; (b) **app entity dictionary** — the app hands the broker the user's own domain entities (contact names, client orgs — which it already holds locally) for exact string-replace, catching domain terms generic NER misses.
    - Honest degradation note surfaced; the user's sovereign, informed choice (covers PII-dependent apps).
  - **B3-creator-spec — the dev's ONLY job is a DECLARATION, not code:** in `freehold.json` the creator declares their PII (either "identifier fields: contact.name, contact.org, …" or a one-line hook returning the entity dictionary). No per-call redaction logic, no "build it like BB." Declaring nothing → the generic floor still applies when the toggle is on. **Enforceable (platform executes) + accurate (app declared what's sensitive) + low burden.** ← update the creator spec / build guide.
  - **B3-app (BB):** implement the dictionary hook (its contacts/orgs) so redaction is exact when the buyer enables the toggle; pass a **prefer-on-device hint** for bulk scans (default on-device = zero egress). Satisfies Priya's "confidential by default" without forcing anything.
