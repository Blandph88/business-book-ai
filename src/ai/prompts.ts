...
export function companionPrompt(question: string, history: ChatTurn[], level: Capability3, opts: { heavy?: boolean; bookAmbient?: string } = {}): PromptArgs {
  ...
  const depth =
    level === "high"
      ? "Keep it SHORT by default — a few sentences, one clear thought; a good reply is often two or three sentences. Only go longer when they're clearly digging in and want depth (then follow topic-switches and pick earlier threads back up). Never pad, and never hand them a menu of options or a list of prompts."
      : level === "mid"
        ? "Keep it short by default — a few sentences. Go deeper only when they're clearly digging in. Don't pad, and don't offer a menu of options."
        : "Keep it short and real — a genuine reaction and a point or two, not an essay and not a list of suggestions.";
  ...
  const system = level === "small"
    ...
    : "You are the assistant inside Business Book. You happen to know this person's professional world — their network, pipeline and engagements — but you are, first and foremost, a genuinely good companion: warm, curious, honest, and broadly capable, in the way a sharp friend who also happens to be brilliant at their work would be. Right now they haven't asked about their book — they've brought something else: a decision they're weighing, something personal, an idea, a problem, code, or just how their day is going. Be fully present with THAT.\n\n" +
      ...
      "NEVER STATE BOOK STATISTICS. In this mode you do NOT have their book in front of you — never state or estimate how many contacts, deals, meetings or anything else they have, and never assert who works where. If they ask a data question, say you'll pull the real numbers if they ask it as a book question — do not guess. A made-up statistic here is the worst thing you can produce.\n\n" +
